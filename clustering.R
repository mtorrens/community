...
G8 <- graph_from_adjacency_matrix(abs(cor(X8)), weighted = w, mode = m)
...
repeat {
  aux <- try(comm.detection(X0, G0, 8, short = TRUE, truth = 1:9, silent = TRUE))
  if (class(aux) != 'try-error') { break }
}
repeat {
  aux <- try(comm.detection(X1, G1, 3, short = TRUE, truth = rep(1, 9)), silent = TRUE)
  if (class(aux) != 'try-error') { break }
}
...
G8 <- graph_from_adjacency_matrix(round(abs(cov(X8)), 2), weighted = w, mode = m)
